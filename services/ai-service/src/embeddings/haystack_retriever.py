...
class HaystackQdrantRetriever:
    ...
    def retrieve_documents(self, query: str, top_k: int | None = None) -> list[dict]:
        """
...
def create_retrieval_pipeline(
    qdrant_client: QdrantClient, collection_name: str, embedder: GemmaEmbedder, top_k: int = 5
    ...
    return HaystackQdrantRetriever(
        qdrant_client=qdrant_client, collection_name=collection_name, embedder=embedder, top_k=top_k
    )


def retrieve_documents(
    retriever: HaystackQdrantRetriever, query: str, top_k: int | None = None
) -> list[dict]:
    """
